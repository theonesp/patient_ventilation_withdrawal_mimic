...
cohort_with_propofol_infusion_epoch<-propofol_infusion_after_ext %>%
  mutate(
    propofol_infusion_epoch_01=if_else(time_from_ext_to_propofol_infusion_min>=0 & time_from_ext_to_propofol_infusion_min<=60,rate,NULL)
    ,propofol_infusion_epoch_02=if_else(time_from_ext_to_propofol_infusion_min>60 & time_from_ext_to_propofol_infusion_min<=120,rate,NULL)
  )
...
cohort_with_propofol_bolus_epoch<-propofol_bolus_after_ext %>%
  mutate(
 propofol_bolus_epoch_01=if_else(time_from_ext_to_propofol_bolus_min>=0 & time_from_ext_to_propofol_bolus_min<=60,amount,NULL)
,propofol_bolus_epoch_02=if_else(time_from_ext_to_propofol_bolus_min>60 & time_from_ext_to_propofol_bolus_min<=120,amount,NULL)
    )
...
# we noticed some timezones adrugs_infusion alteterd, so we drugs_infusionset them
diazepam_infusion_from_patients_tofilter$charttime_drugs_infusion<-as.character(diazepam_infusion_from_patients_tofilter$charttime_drugs_infusion)
...
diazepam_infusion_from_patients_tofilter$charttime_drugs_infusion<-as.POSIXct(diazepam_infusion_from_patients_tofilter$charttime_drugs_infusion, format='%Y-%m-%d %H:%M:%S', tz="GMT")
...
diazepam_infusion_from_patients_tofilter['time_from_ext_to_diazepam_infusion_min']<-difftime(diazepam_infusion_from_patients_tofilter$charttime_drugs_infusion,diazepam_infusion_from_patients_tofilter$charttime.extubation,units = 'mins')


#Drug Push is Bolus, so I filtered them out
diazepam_infusion_after_ext<-diazepam_infusion_from_patients_tofilter %>%
  filter( 
    time_from_ext_to_diazepam_infusion_min >=0 
  )

# diazepam =  1 milligram of morphine IV (225154) is equivalent to .01 mg Fentanyl IV (221744).

n_distinct(diazepam_infusion_after_ext$subject_id) #84 patients 1/7/18

library(dplyr)
## if(any(gdrugs_infusionpl("package:plyr", search()))) detach("package:plyr") else message("plyr not loaded")
cohort_with_diazepam_infusion_epoch<-diazepam_infusion_after_ext %>%
  mutate(
    diazepam_infusion_epoch_01=if_else(time_from_ext_to_diazepam_infusion_min>=0 & time_from_ext_to_diazepam_infusion_min<=60,rate,NULL)
    ,diazepam_infusion_epoch_02=if_else(time_from_ext_to_diazepam_infusion_min>60 & time_from_ext_to_diazepam_infusion_min<=120,rate,NULL)
  )
...
diazepam_bolus_after_ext<-diazepam_bolus_from_patients_tofilter %>%
  filter( 
          time_from_ext_to_diazepam_bolus_min >=0 
          )
...
# 322 patients with time_from_ext_to_diazepam_bolus_min >=0 2/12/19
...
cohort_with_diazepam_bolus_epoch<-diazepam_bolus_after_ext %>%
  mutate(
 diazepam_bolus_epoch_01=if_else(time_from_ext_to_diazepam_bolus_min>=0 & time_from_ext_to_diazepam_bolus_min<=60,amount,NULL)
,diazepam_bolus_epoch_02=if_else(time_from_ext_to_diazepam_bolus_min>60 & time_from_ext_to_diazepam_bolus_min<=120,amount,NULL)
    )
...
# we noticed some timezones adrugs_infusion alteterd, so we drugs_infusionset them
midazolam_infusion_from_patients_tofilter$charttime_drugs_infusion<-as.character(midazolam_infusion_from_patients_tofilter$charttime_drugs_infusion)
...
midazolam_infusion_from_patients_tofilter$charttime_drugs_infusion<-as.POSIXct(midazolam_infusion_from_patients_tofilter$charttime_drugs_infusion, format='%Y-%m-%d %H:%M:%S', tz="GMT")
...
midazolam_infusion_from_patients_tofilter['time_from_ext_to_midazolam_infusion_min']<-difftime(midazolam_infusion_from_patients_tofilter$charttime_drugs_infusion,midazolam_infusion_from_patients_tofilter$charttime.extubation,units = 'mins')


#Drug Push is Bolus, so I filtered them out
midazolam_infusion_after_ext<-midazolam_infusion_from_patients_tofilter %>%
  filter( 
    time_from_ext_to_midazolam_infusion_min >=0 
  )



n_distinct(midazolam_infusion_after_ext$subject_id) #84 patients 1/7/18

library(dplyr)
## if(any(gdrugs_infusionpl("package:plyr", search()))) detach("package:plyr") else message("plyr not loaded")
cohort_with_midazolam_infusion_epoch<-midazolam_infusion_after_ext %>%
  mutate(
    midazolam_infusion_epoch_01=if_else(time_from_ext_to_midazolam_infusion_min>=0 & time_from_ext_to_midazolam_infusion_min<=60,rate,NULL)
    ,midazolam_infusion_epoch_02=if_else(time_from_ext_to_midazolam_infusion_min>60 & time_from_ext_to_midazolam_infusion_min<=120,rate,NULL)
  )
...
midazolam_bolus_after_ext<-midazolam_bolus_from_patients_tofilter %>%
  filter( 
          time_from_ext_to_midazolam_bolus_min >=0 
          )
...
# 322 patients with time_from_ext_to_midazolam_bolus_min >=0 2/12/19
...
cohort_with_midazolam_bolus_epoch<-midazolam_bolus_after_ext %>%
  mutate(
 midazolam_bolus_epoch_01=if_else(time_from_ext_to_midazolam_bolus_min>=0 & time_from_ext_to_midazolam_bolus_min<=60,amount,NULL)
,midazolam_bolus_epoch_02=if_else(time_from_ext_to_midazolam_bolus_min>60 & time_from_ext_to_midazolam_bolus_min<=120,amount,NULL)
    )
...
# we noticed some timezones adrugs_infusion alteterd, so we drugs_infusionset them
lorazepam_infusion_from_patients_tofilter$charttime_drugs_infusion<-as.character(lorazepam_infusion_from_patients_tofilter$charttime_drugs_infusion)
...
lorazepam_infusion_from_patients_tofilter$charttime_drugs_infusion<-as.POSIXct(lorazepam_infusion_from_patients_tofilter$charttime_drugs_infusion, format='%Y-%m-%d %H:%M:%S', tz="GMT")
...
lorazepam_infusion_from_patients_tofilter['time_from_ext_to_lorazepam_infusion_min']<-difftime(lorazepam_infusion_from_patients_tofilter$charttime_drugs_infusion,lorazepam_infusion_from_patients_tofilter$charttime.extubation,units = 'mins')


#Drug Push is Bolus, so I filtered them out
lorazepam_infusion_after_ext<-lorazepam_infusion_from_patients_tofilter %>%
  filter( 
    time_from_ext_to_lorazepam_infusion_min >=0 
  )

# lorazepam =  1 milligram of morphine IV (225154) is equivalent to .01 mg Fentanyl IV (221744).

n_distinct(lorazepam_infusion_after_ext$subject_id) #84 patients 1/7/18

library(dplyr)
## if(any(gdrugs_infusionpl("package:plyr", search()))) detach("package:plyr") else message("plyr not loaded")
cohort_with_lorazepam_infusion_epoch<-lorazepam_infusion_after_ext %>%
  mutate(
    lorazepam_infusion_epoch_01=if_else(time_from_ext_to_lorazepam_infusion_min>=0 & time_from_ext_to_lorazepam_infusion_min<=60,rate,NULL)
    ,lorazepam_infusion_epoch_02=if_else(time_from_ext_to_lorazepam_infusion_min>60 & time_from_ext_to_lorazepam_infusion_min<=120,rate,NULL)
  )
...
lorazepam_bolus_after_ext<-lorazepam_bolus_from_patients_tofilter %>%
  filter( 
          time_from_ext_to_lorazepam_bolus_min >=0 
          )
...
# 322 patients with time_from_ext_to_lorazepam_bolus_min >=0 2/12/19
...
cohort_with_lorazepam_bolus_epoch<-lorazepam_bolus_after_ext %>%
  mutate(
 lorazepam_bolus_epoch_01=if_else(time_from_ext_to_lorazepam_bolus_min>=0 & time_from_ext_to_lorazepam_bolus_min<=60,amount,NULL)
,lorazepam_bolus_epoch_02=if_else(time_from_ext_to_lorazepam_bolus_min>60 & time_from_ext_to_lorazepam_bolus_min<=120,amount,NULL)
    )
...
# we noticed some timezones adrugs_infusion alteterd, so we drugs_infusionset them
dexmedetomedie_infusion_from_patients_tofilter$charttime_drugs_infusion<-as.character(dexmedetomedie_infusion_from_patients_tofilter$charttime_drugs_infusion)
...
dexmedetomedie_infusion_from_patients_tofilter$charttime_drugs_infusion<-as.POSIXct(dexmedetomedie_infusion_from_patients_tofilter$charttime_drugs_infusion, format='%Y-%m-%d %H:%M:%S', tz="GMT")
...
dexmedetomedie_infusion_from_patients_tofilter['time_from_ext_to_dexmedetomedie_infusion_min']<-difftime(dexmedetomedie_infusion_from_patients_tofilter$charttime_drugs_infusion,dexmedetomedie_infusion_from_patients_tofilter$charttime.extubation,units = 'mins')


#Drug Push is Bolus, so I filtered them out
dexmedetomedie_infusion_after_ext<-dexmedetomedie_infusion_from_patients_tofilter %>%
  filter( 
    time_from_ext_to_dexmedetomedie_infusion_min >=0 
  )

# dexmedetomedie =  1 milligram of morphine IV (225154) is equivalent to .01 mg Fentanyl IV (221744).

n_distinct(dexmedetomedie_infusion_after_ext$subject_id) #84 patients 1/7/18

library(dplyr)
## if(any(gdrugs_infusionpl("package:plyr", search()))) detach("package:plyr") else message("plyr not loaded")
cohort_with_dexmedetomedie_infusion_epoch<-dexmedetomedie_infusion_after_ext %>%
  mutate(
    dexmedetomedie_infusion_epoch_01=if_else(time_from_ext_to_dexmedetomedie_infusion_min>=0 & time_from_ext_to_dexmedetomedie_infusion_min<=60,rate,NULL)
    ,dexmedetomedie_infusion_epoch_02=if_else(time_from_ext_to_dexmedetomedie_infusion_min>60 & time_from_ext_to_dexmedetomedie_infusion_min<=120,rate,NULL)
  )
...
dexmedetomedie_bolus_after_ext<-dexmedetomedie_bolus_from_patients_tofilter %>%
  filter( 
          time_from_ext_to_dexmedetomedie_bolus_min >=0 
          )
...
# 322 patients with time_from_ext_to_dexmedetomedie_bolus_min >=0 2/12/19
...
cohort_with_dexmedetomedie_bolus_epoch<-dexmedetomedie_bolus_after_ext %>%
  mutate(
 dexmedetomedie_bolus_epoch_01=if_else(time_from_ext_to_dexmedetomedie_bolus_min>=0 & time_from_ext_to_dexmedetomedie_bolus_min<=60,amount,NULL)
,dexmedetomedie_bolus_epoch_02=if_else(time_from_ext_to_dexmedetomedie_bolus_min>60 & time_from_ext_to_dexmedetomedie_bolus_min<=120,amount,NULL)
    )
...
propofol_infusion_before_ext<-propofol_infusion_from_patients_tofilter %>%
  filter( 
          time_from_ext_to_propofol_infusion_min >=-12*60 
          )

# propofol =  1 milligram of morphine IV (225154) is equivalent to .01 mg Fentanyl IV (221744).

n_distinct(propofol_infusion_before_ext$subject_id) #545 patients 1/7/18

library(dplyr)

cohort_with_propofol_infusion_neg_epoch<-propofol_infusion_before_ext %>%
  mutate(
 propofol_infusion_neg_epoch_01=if_else( between (time_from_ext_to_propofol_infusion_min , -60,   0  ),rate,NULL)
,propofol_infusion_neg_epoch_02=if_else( between (time_from_ext_to_propofol_infusion_min ,-120, -60  ),rate,NULL)
    )
...
propofol_bolus_before_ext<-propofol_bolus_from_patients_tofilter %>%
  filter( 
          time_from_ext_to_propofol_bolus_min >=-12*60 
          )
...
cohort_with_propofol_bolus_neg_epoch<-propofol_bolus_before_ext %>%
  mutate(
 propofol_bolus_neg_epoch_01=if_else(between(time_from_ext_to_propofol_bolus_min, -60,   0) ,amount,NULL)
,propofol_bolus_neg_epoch_02=if_else(between(time_from_ext_to_propofol_bolus_min,-120, -60) ,amount,NULL)
    )
...
diazepam_infusion_before_ext<-diazepam_infusion_from_patients_tofilter %>%
  filter( 
          time_from_ext_to_diazepam_infusion_min >=-12*60 
          )

# diazepam =  1 milligram of morphine IV (225154) is equivalent to .01 mg Fentanyl IV (221744).

n_distinct(diazepam_infusion_before_ext$subject_id) #545 patients 1/7/18

library(dplyr)

cohort_with_diazepam_infusion_neg_epoch<-diazepam_infusion_before_ext %>%
  mutate(
 diazepam_infusion_neg_epoch_01=if_else( between (time_from_ext_to_diazepam_infusion_min , -60,   0  ),rate,NULL)
,diazepam_infusion_neg_epoch_02=if_else( between (time_from_ext_to_diazepam_infusion_min ,-120, -60  ),rate,NULL)
    )
...
diazepam_bolus_before_ext<-diazepam_bolus_from_patients_tofilter %>%
  filter( 
          time_from_ext_to_diazepam_bolus_min >=-12*60 
          )
...
cohort_with_diazepam_bolus_neg_epoch<-diazepam_bolus_before_ext %>%
  mutate(
 diazepam_bolus_neg_epoch_01=if_else(between(time_from_ext_to_diazepam_bolus_min, -60,   0) ,amount,NULL)
,diazepam_bolus_neg_epoch_02=if_else(between(time_from_ext_to_diazepam_bolus_min,-120, -60) ,amount,NULL)
    )
...
midazolam_infusion_before_ext<-midazolam_infusion_from_patients_tofilter %>%
  filter( 
          time_from_ext_to_midazolam_infusion_min >=-12*60 
          )



n_distinct(midazolam_infusion_before_ext$subject_id) #545 patients 1/7/18

library(dplyr)

cohort_with_midazolam_infusion_neg_epoch<-midazolam_infusion_before_ext %>%
  mutate(
 midazolam_infusion_neg_epoch_01=if_else( between (time_from_ext_to_midazolam_infusion_min , -60,   0  ),rate,NULL)
,midazolam_infusion_neg_epoch_02=if_else( between (time_from_ext_to_midazolam_infusion_min ,-120, -60  ),rate,NULL)
    )
...
midazolam_bolus_before_ext<-midazolam_bolus_from_patients_tofilter %>%
  filter( 
          time_from_ext_to_midazolam_bolus_min >=-12*60 
          )
...
cohort_with_midazolam_bolus_neg_epoch<-midazolam_bolus_before_ext %>%
  mutate(
 midazolam_bolus_neg_epoch_01=if_else(between(time_from_ext_to_midazolam_bolus_min, -60,   0) ,amount,NULL)
,midazolam_bolus_neg_epoch_02=if_else(between(time_from_ext_to_midazolam_bolus_min,-120, -60) ,amount,NULL)
    )
...
lorazepam_infusion_before_ext<-lorazepam_infusion_from_patients_tofilter %>%
  filter( 
          time_from_ext_to_lorazepam_infusion_min >=-12*60 
          )

# lorazepam =  1 milligram of morphine IV (225154) is equivalent to .01 mg Fentanyl IV (221744).

n_distinct(lorazepam_infusion_before_ext$subject_id) #545 patients 1/7/18

library(dplyr)

cohort_with_lorazepam_infusion_neg_epoch<-lorazepam_infusion_before_ext %>%
  mutate(
 lorazepam_infusion_neg_epoch_01=if_else( between (time_from_ext_to_lorazepam_infusion_min , -60,   0  ),rate,NULL)
,lorazepam_infusion_neg_epoch_02=if_else( between (time_from_ext_to_lorazepam_infusion_min ,-120, -60  ),rate,NULL)
    )
...
lorazepam_bolus_before_ext<-lorazepam_bolus_from_patients_tofilter %>%
  filter( 
          time_from_ext_to_lorazepam_bolus_min >=-12*60 
          )
...
cohort_with_lorazepam_bolus_neg_epoch<-lorazepam_bolus_before_ext %>%
  mutate(
 lorazepam_bolus_neg_epoch_01=if_else(between(time_from_ext_to_lorazepam_bolus_min, -60,   0) ,amount,NULL)
,lorazepam_bolus_neg_epoch_02=if_else(between(time_from_ext_to_lorazepam_bolus_min,-120, -60) ,amount,NULL)
    )
...
dexmedetomedie_infusion_before_ext<-dexmedetomedie_infusion_from_patients_tofilter %>%
  filter( 
          time_from_ext_to_dexmedetomedie_infusion_min >=-12*60 
          )

# dexmedetomedie =  1 milligram of morphine IV (225154) is equivalent to .01 mg Fentanyl IV (221744).

n_distinct(dexmedetomedie_infusion_before_ext$subject_id) #545 patients 1/7/18

library(dplyr)

cohort_with_dexmedetomedie_infusion_neg_epoch<-dexmedetomedie_infusion_before_ext %>%
  mutate(
 dexmedetomedie_infusion_neg_epoch_01=if_else( between (time_from_ext_to_dexmedetomedie_infusion_min , -60,   0  ),rate,NULL)
,dexmedetomedie_infusion_neg_epoch_02=if_else( between (time_from_ext_to_dexmedetomedie_infusion_min ,-120, -60  ),rate,NULL)
    )
...
dexmedetomedie_bolus_before_ext<-dexmedetomedie_bolus_from_patients_tofilter %>%
  filter( 
          time_from_ext_to_dexmedetomedie_bolus_min >=-12*60 
          )
...
cohort_with_dexmedetomedie_bolus_neg_epoch<-dexmedetomedie_bolus_before_ext %>%
  mutate(
 dexmedetomedie_bolus_neg_epoch_01=if_else(between(time_from_ext_to_dexmedetomedie_bolus_min, -60,   0) ,amount,NULL)
,dexmedetomedie_bolus_neg_epoch_02=if_else(between(time_from_ext_to_dexmedetomedie_bolus_min,-120, -60) ,amount,NULL)
    )
...
phenylephrine_infusion_cohort_24$itemid[propofol_infusion_cohort_24$itemid==221749]<-'phenylephrine'
#Valium (Diazepam)
#We tried valium and it didn't increase the number of patients, still 84 after ext
```
...
# we noticed some timezones adrugs_infusion alteterd, so we drugs_infusionset them
phenylephrine_infusion_from_patients_tofilter$charttime_drugs_infusion<-as.character(phenylephrine_infusion_from_patients_tofilter$charttime_drugs_infusion)
...
phenylephrine_infusion_from_patients_tofilter$charttime_drugs_infusion<-as.POSIXct(phenylephrine_infusion_from_patients_tofilter$charttime_drugs_infusion, format='%Y-%m-%d %H:%M:%S', tz="GMT")
...
phenylephrine_infusion_from_patients_tofilter['time_from_ext_to_phenylephrine_infusion_min']<-difftime(phenylephrine_infusion_from_patients_tofilter$charttime_drugs_infusion,phenylephrine_infusion_from_patients_tofilter$charttime.extubation,units = 'mins')
```
...
phenylephrine_infusion_before_ext<-phenylephrine_infusion_from_patients_tofilter %>%
  filter( 
          time_from_ext_to_phenylephrine_infusion_min >=-12*60 
          )

# phenylephrine =  1 milligram of morphine IV (225154) is equivalent to .01 mg Fentanyl IV (221744).

n_distinct(phenylephrine_infusion_before_ext$subject_id) #545 patients 1/7/18

library(dplyr)

cohort_with_phenylephrine_infusion_neg_epoch<-phenylephrine_infusion_before_ext %>%
  mutate(
 phenylephrine_infusion_neg_epoch_01=if_else( between (time_from_ext_to_phenylephrine_infusion_min , -60,   0  ),rate,NULL)
,phenylephrine_infusion_neg_epoch_02=if_else( between (time_from_ext_to_phenylephrine_infusion_min ,-120, -60  ),rate,NULL)
    )
...
norepinephrine_infusion_cohort_24$itemid[propofol_infusion_cohort_24$itemid==221906]<-'norepinephrine'
#Valium (Diazepam)
#We tried valium and it didn't increase the number of patients, still 84 after ext
```
...
# we noticed some timezones adrugs_infusion alteterd, so we drugs_infusionset them
norepinephrine_infusion_from_patients_tofilter$charttime_drugs_infusion<-as.character(norepinephrine_infusion_from_patients_tofilter$charttime_drugs_infusion)
...
norepinephrine_infusion_from_patients_tofilter$charttime_drugs_infusion<-as.POSIXct(norepinephrine_infusion_from_patients_tofilter$charttime_drugs_infusion, format='%Y-%m-%d %H:%M:%S', tz="GMT")
...
norepinephrine_infusion_from_patients_tofilter['time_from_ext_to_norepinephrine_infusion_min']<-difftime(norepinephrine_infusion_from_patients_tofilter$charttime_drugs_infusion,norepinephrine_infusion_from_patients_tofilter$charttime.extubation,units = 'mins')
```
...
norepinephrine_infusion_before_ext<-norepinephrine_infusion_from_patients_tofilter %>%
  filter( 
          time_from_ext_to_norepinephrine_infusion_min >=-12*60 
          )

# norepinephrine =  1 milligram of morphine IV (225154) is equivalent to .01 mg Fentanyl IV (221744).

n_distinct(norepinephrine_infusion_before_ext$subject_id) #545 patients 1/7/18

library(dplyr)

cohort_with_norepinephrine_infusion_neg_epoch<-norepinephrine_infusion_before_ext %>%
  mutate(
 norepinephrine_infusion_neg_epoch_01=if_else( between (time_from_ext_to_norepinephrine_infusion_min , -60,   0  ),rate,NULL)
,norepinephrine_infusion_neg_epoch_02=if_else( between (time_from_ext_to_norepinephrine_infusion_min ,-120, -60  ),rate,NULL)
    )
...
dopamine_infusion_cohort_24$itemid[propofol_infusion_cohort_24$itemid==221662]<-'dopamine'
#Valium (Diazepam)
#We tried valium and it didn't increase the number of patients, still 84 after ext
```
...
# we noticed some timezones adrugs_infusion alteterd, so we drugs_infusionset them
dopamine_infusion_from_patients_tofilter$charttime_drugs_infusion<-as.character(dopamine_infusion_from_patients_tofilter$charttime_drugs_infusion)
...
dopamine_infusion_from_patients_tofilter$charttime_drugs_infusion<-as.POSIXct(dopamine_infusion_from_patients_tofilter$charttime_drugs_infusion, format='%Y-%m-%d %H:%M:%S', tz="GMT")
...
dopamine_infusion_from_patients_tofilter['time_from_ext_to_dopamine_infusion_min']<-difftime(dopamine_infusion_from_patients_tofilter$charttime_drugs_infusion,dopamine_infusion_from_patients_tofilter$charttime.extubation,units = 'mins')
```
...
dopamine_infusion_before_ext<-dopamine_infusion_from_patients_tofilter %>%
  filter( 
          time_from_ext_to_dopamine_infusion_min >=-12*60 
          )

# dopamine =  1 milligram of morphine IV (225154) is equivalent to .01 mg Fentanyl IV (221744).

n_distinct(dopamine_infusion_before_ext$subject_id) #545 patients 1/7/18

library(dplyr)

cohort_with_dopamine_infusion_neg_epoch<-dopamine_infusion_before_ext %>%
  mutate(
 dopamine_infusion_neg_epoch_01=if_else( between (time_from_ext_to_dopamine_infusion_min , -60,   0  ),rate,NULL)
,dopamine_infusion_neg_epoch_02=if_else( between (time_from_ext_to_dopamine_infusion_min ,-120, -60  ),rate,NULL)
    )
...
epinephrine_infusion_cohort_24$itemid[propofol_infusion_cohort_24$itemid==221289]<-'epinephrine'
#Valium (Diazepam)
#We tried valium and it didn't increase the number of patients, still 84 after ext
```
...
# we noticed some timezones adrugs_infusion alteterd, so we drugs_infusionset them
epinephrine_infusion_from_patients_tofilter$charttime_drugs_infusion<-as.character(epinephrine_infusion_from_patients_tofilter$charttime_drugs_infusion)
...
epinephrine_infusion_from_patients_tofilter$charttime_drugs_infusion<-as.POSIXct(epinephrine_infusion_from_patients_tofilter$charttime_drugs_infusion, format='%Y-%m-%d %H:%M:%S', tz="GMT")
...
epinephrine_infusion_from_patients_tofilter['time_from_ext_to_epinephrine_infusion_min']<-difftime(epinephrine_infusion_from_patients_tofilter$charttime_drugs_infusion,epinephrine_infusion_from_patients_tofilter$charttime.extubation,units = 'mins')
```
...
epinephrine_infusion_before_ext<-epinephrine_infusion_from_patients_tofilter %>%
  filter( 
          time_from_ext_to_epinephrine_infusion_min >=-12*60 
          )

# epinephrine =  1 milligram of morphine IV (225154) is equivalent to .01 mg Fentanyl IV (221744).

n_distinct(epinephrine_infusion_before_ext$subject_id) #545 patients 1/7/18

library(dplyr)

cohort_with_epinephrine_infusion_neg_epoch<-epinephrine_infusion_before_ext %>%
  mutate(
 epinephrine_infusion_neg_epoch_01=if_else( between (time_from_ext_to_epinephrine_infusion_min , -60,   0  ),rate,NULL)
,epinephrine_infusion_neg_epoch_02=if_else( between (time_from_ext_to_epinephrine_infusion_min ,-120, -60  ),rate,NULL)
    )
...
vasopressin_infusion_cohort_24$itemid[propofol_infusion_cohort_24$itemid==222315]<-'vasopressin'
#Valium (Diazepam)
#We tried valium and it didn't increase the number of patients, still 84 after ext
```
...
# we noticed some timezones adrugs_infusion alteterd, so we drugs_infusionset them
vasopressin_infusion_from_patients_tofilter$charttime_drugs_infusion<-as.character(vasopressin_infusion_from_patients_tofilter$charttime_drugs_infusion)
...
vasopressin_infusion_from_patients_tofilter$charttime_drugs_infusion<-as.POSIXct(vasopressin_infusion_from_patients_tofilter$charttime_drugs_infusion, format='%Y-%m-%d %H:%M:%S', tz="GMT")
...
vasopressin_infusion_from_patients_tofilter['time_from_ext_to_vasopressin_infusion_min']<-difftime(vasopressin_infusion_from_patients_tofilter$charttime_drugs_infusion,vasopressin_infusion_from_patients_tofilter$charttime.extubation,units = 'mins')
```
...
vasopressin_infusion_before_ext<-vasopressin_infusion_from_patients_tofilter %>%
  filter( 
          time_from_ext_to_vasopressin_infusion_min >=-12*60 
          )

# vasopressin =  1 milligram of morphine IV (225154) is equivalent to .01 mg Fentanyl IV (221744).

n_distinct(vasopressin_infusion_before_ext$subject_id) #545 patients 1/7/18

library(dplyr)

cohort_with_vasopressin_infusion_neg_epoch<-vasopressin_infusion_before_ext %>%
  mutate(
 vasopressin_infusion_neg_epoch_01=if_else( between (time_from_ext_to_vasopressin_infusion_min , -60,   0  ),rate,NULL)
,vasopressin_infusion_neg_epoch_02=if_else( between (time_from_ext_to_vasopressin_infusion_min ,-120, -60  ),rate,NULL)
    )
